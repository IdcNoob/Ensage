namespace InformationPinger
{
    ...
    internal class MenuManager
    {
        ...
        public MenuManager()
        {
            ...
            itemMenu.AddItem(
                forcePing = new MenuItem("forcePing", "Force ping:").SetValue(
                    new AbilityToggler(
            ...
            itemMenu.AddItem(
                bottleRune = new MenuItem("bottleRune", "Bottled rune:").SetValue(
                    new AbilityToggler(
                        new Dictionary<string, bool>
                        {
            ...
            abilityMenu
                .AddItem(rubickDisable = new MenuItem("rubickDisable", "Rubick's stolen disable").SetValue(false))
                .SetTooltip("Will ping if rubick stole disable ability");
            abilityMenu
                .AddItem(rubickUltimate = new MenuItem("rubickUltimate", "Rubick's stolen ultimate").SetValue(false))
                .SetTooltip("Will ping if rubick stole ultimate ability");
            abilityMenu
                .AddItem(abilityEnemyCheck = new MenuItem("abilityEnemyCheck", "Check for enemies").SetValue(false))
        ...
        public List<RuneType> BottleRunes()
        {
            ...
            foreach (var rune in bottleRune.GetValue<AbilityToggler>()
                .Dictionary.Where(x => x.Value)
                .Select(x => x.Key))
            {
                switch (rune)
                {
